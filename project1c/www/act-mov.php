              ...
              <div class="col-lg-12">
                <div class="form-panel">
                  ...
                  <form class="form-horizontal style-form" method="get" action="<?php echo ($_SERVER["PHP_SELF"]);?>">
                    ...
                    <div class="form-group">
                      <label class="col-sm-2 control-label">Actor Name</label>
                      <div class="col-sm-7">
                        <select   class="form-control" name="name">
                          ...
                          if($db->connect_errno > 0)
                            die('Unable to connect to database [' . $db->connect_error . ']');
                          $query = "SELECT id, last, first FROM Actor ORDER BY first, last;";
                          if(!$result = $db->query($query))
                            echo "Invalid query.<br>";
                          else{
                            while($row=$result->fetch_row())
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-7">
                        <input type="text" class="form-control" placeholder="Please type a role..." name="role">
                        <span class="help-block">EX. Detective</span>
                      </div>
                    ...
                    if(!$incomplete) {
                      ...
                      $mid = $_GET["title"];
                      $aid = $_GET["name"];
                      $query = "INSERT INTO MovieActor VALUES (".$mid.",".$aid.",'".$_GET["role"]."');";
                        //echo $query;
                      if(!$result = $db->query($query))
                        echo "The input relation is already in the database.<br>";
                      else
                         // Insert new relation to MovieActor table 
                        echo "Success!";
                      $result->free();
                      $db->close();
                      printf('</div>');
