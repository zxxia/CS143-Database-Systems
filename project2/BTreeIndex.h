...
class BTreeIndex {
 public:
  ...
  //void printTree(PageId root_pid, int height);
 ...
 private:
  ...
  // private functions to do recursion
  RC findandoverflow(int h, int& key, const RecordId& rid, PageId pid, PageId& leftPid, PageId& rightPid);

  RC re_locate(int searchKey, PageId pid, int h, IndexCursor &cursor);
};
